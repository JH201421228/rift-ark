...
✅ 데이터        game/data/ads.json  (배수 2.0 · 하루 1회 · 6스테이지부터)
✅ 플러그인      @capacitor-community/admob@7.2.0  (Capacitor 7 계열)
✅ 네이티브      AndroidManifest APPLICATION_ID · Info.plist GADApplicationIdentifier
                 (지금은 **구글 공식 테스트 앱 ID**)
✅ 하네스        npm run economy 이 광고를 켠 파워 곡선을 함께 잰다
✅ APK           riftark-20260807c.apk — SDK 포함 확인 (33MB → 38MB)

❌ AdMob 계정 · 광고 단위 ID
❌ UMP 동의 메시지
❌ 개인정보 처리방침 (지금 문안은 "아무것도 수집하지 않습니다" — 거짓이 된다)
❌ git 원격 · 릴리스 서명 키 · CI       ← 이것들이 여전히 임계 경로다 (35-remaining-work-roadmap R-01)
❌ 경제 재보정 결정                      ← §2 에서 반드시 결정해야 한다
```
...
**40–60 의 벽이 사라진다.** 어떤 편성으로 들어가도 통과한다.

### 2.2 선택지 — 하나를 골라야 한다

| | 무엇 | 광고 수익 | 대가 |
|---|---|---|---|
| **A** | 수치를 낮춘다 (×1.4 · 하루 2회 → 파워 +19%, 게이트 통과) | 낮다 (하루 2회) | 보상이 작아 시청률이 떨어진다 |
| **B** | **2배를 유지하고 100 스테이지를 재보정한다** | 높다 | `difficultyMult` 100개 재조정 + `balance:check` 300시드(약 2시간) 재검증. **밸런싱 한 사이클** |
| **C** | 골드가 아닌 보상으로 바꾼다 (예: 그 판의 각인 리롤 +1) | **가장 높다** — 하루 상한이 필요 없어 매 전투마다 제안할 수 있다 | 리롤은 기록보관소 시설과 축이 겹친다. 별도 설계 판단 필요 |

> **사용자는 2배(=B)를 선택했다.** 그래서 `ads.json` 은 `rewardMult: 2.0` 이다.
> 다만 **재보정은 아직 하지 않았다** — 그것이 §2.3 이다.
