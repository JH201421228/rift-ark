...
| | |
|---|---|
| 원격 | `github.com/JH201421228/rift-ark` (**public** — 2026-08-08 실측) |
...
> ### ★ 저장소는 **public 이다** — 그리고 그 편이 낫다 (2026-08-08 정정)
>
> 이 자리에는 *"private 로 만든다. 광고를 붙이면 AdMob 광고 단위 ID 가 저장소에
> 들어가고, 남이 자기 앱에 붙여 무효 트래픽을 만들 수 있다"* 고 적혀 있었다.
> **실제 저장소는 public 이고, 그 근거도 과장이었다.**
>
> | | |
> |---|---|
> | AdMob ID 노출 | **저장소 공개와 거의 무관하다.** 앱 ID·광고 단위 ID 는 **배포된 APK 에서 누구나 추출**할 수 있다 — 스토어에 올린 순간 공개나 마찬가지다. 비밀이 아니며 Google 도 비밀로 다루지 않는다 |
> | 진짜 방어선 | AdMob 의 무효 트래픽 탐지와 `app-ads.txt` 다. 저장소를 잠근다고 생기지 않는다 |
> | ★ public 의 실익 | **GitHub Actions 가 무료다 — macOS 러너까지.** private 이면 macOS 는 Linux 의 10배 분을 먹는다 |
>
> ⚠ **대신 비밀은 한 톨도 들어가면 안 된다.** 2026-08-08 에 전수 확인했다 —
> 커밋·히스토리 어디에도 `*.jks` · `key.properties` · 서비스 계정 JSON 이 없다.
> `.gitignore` 가 막고 있고, `git check-ignore -v <경로>` 가 권위다.
...
## 3. GCP 서비스 계정 — ★ **지금 만든다** (2026-08-08, 완전 CI/CD 결정)

> ### ★★★ 이것은 **Codemagic 의 요구가 아니라 Play 의 요구다**
>
> 예전 문장은 *"Codemagic 이 Play 에 AAB 를 자동 업로드하려면"* 이라 **도구의
> 요구처럼 읽혔다.** 아니다 — **Play Developer API 에 인증하는 유일한 수단**이고,
> Codemagic · GitHub Actions · fastlane · 직접 짠 스크립트 **전부** 이것이 있어야 한다.
> CI 도구를 바꿔도 사라지지 않는다.
>
> | 없애는 유일한 방법 | 자동 업로드를 **안 하는 것** — CI 는 빌드만 하고 AAB 를 아티팩트로 받아 손으로 올린다 |
> |---|---|
> | 이 프로젝트의 선택 | **자동 업로드까지 한다** (2026-08-08 사용자 결정). 그래서 만든다 |
>
> ⚠ **이 JSON 은 곧 업로드 권한이다.** 저장소에 넣지 않는다 — Codemagic 의
> 암호화된 환경변수에만 둔다 (§9.3). 저장소가 **public** 이라 더욱 그렇다.

Play 에 AAB 를 자동 업로드하려면 **서비스 계정 JSON 키**가 필요하다.
...
### ★ 아래 §9.1–9.6 을 **지금 실행한다** (2026-08-08, 완전 CI/CD 결정)

`codemagic.yaml` 은 저장소에 이미 있고 검증도 끝났다 — 남은 것은 **연결**뿐이다.

> ### ★★ Android 와 iOS 를 **둘 다 Codemagic 으로** 한다 (사용자 결정)
>
> 저장소가 public 이라 GitHub Actions 도 macOS 러너까지 무료다. 그런데도
> Codemagic 을 고른 이유는 **애플 코드 서명**이다 — 사용자에게 맥이 없다.
>
> | | |
> |---|---|
> | 맥이 필요한가 | ❌ **둘 다 아니다.** GH Actions 의 macOS 러너도 클라우드의 진짜 맥이다 |
> | 그럼 무엇이 다른가 | GH Actions 는 `.p12` 인증서와 프로비저닝 프로파일을 **직접 내보내** secret 에 넣어야 한다. 맥 없이는 번거롭다 |
> | ★ Codemagic | **App Store Connect API 키만 주면 인증서를 알아서 만들고 갱신한다.** 맥 없는 개발자에게 이것이 실질적 이점이다 |
>
> ⚠ **iOS 워크플로는 아직 쓰지 않았다.** 준비물이 빠져 있어 **돌 수 없는
> 워크플로가 되기 때문이다** — 저장소에 그런 것을 두는 것이 이 저장소가 싫어하는
> "선언했는데 아무도 읽지 않는 것"이다. 준비물은 §9.7.
...
### 9.7 ⏸ iOS 워크플로 — **준비물이 갖춰지면 쓴다**

`codemagic.yaml` 에 iOS 워크플로를 **아직 넣지 않았다.** 아래가 하나라도 없으면
그 워크플로는 **돌 수 없고**, 돌지 않는 설정을 저장소에 두는 것이 이 저장소가
반복해서 당한 실패 유형이다 (`CLAUDE.md` — 선언했는데 아무도 읽지 않는 것).

| 준비물 | 상태 (2026-08-08) | 없으면 |
|---|---|---|
| **Apple Developer Program** ($99/년) | ❓ 미확인 | 서명 자체가 불가능하다 |
| **App Store Connect API 키** (.p8) | ❌ | Codemagic 자동 서명의 입력이다 |
| App Store Connect 앱 등록 · 번들 ID | ❌ | 업로드 대상이 없다 |
| **`PrivacyInfo.xcprivacy`** | ❌ **없다** | 2024-05 이후 애플이 **거부**한다 (`56 §4.4` 가 전문) |
| iOS 광고 단위 ID (`ads.json:units.ios`) | ❌ 빈 값 | 테스트 광고로 떨어져 **수익 0** (`check:prod` A1 이 잡는다) |
| `Info.plist` 의 실제 AdMob 앱 ID | ❓ | 앱이 **부팅 즉시 죽는다** |
| ATT(앱 추적 투명성) 판단 | ❌ | `56 §3.3` |

> ★ **`PrivacyInfo.xcprivacy` 와 iOS 광고 ID 는 CI 이전의 문제다.** CI 를 붙여도
> 그 둘이 없으면 심사에서 막힌다. 순서는 **준비물 → 워크플로**다.
>
> ★ 워크플로 자체는 `android-release` 와 같은 모양이 된다 —
> `instance_type: mac_mini_m2` · `ios_signing` · `publishing.app_store_connect`.
> 그때 `50` 이 아니라 **[`51-app-store-paid-codemagic.md`](51-app-store-paid-codemagic.md)**
> 가 정본이다 (⚠ 그 문서는 아직 **유료 앱 기준**으로 쓰여 있다).
