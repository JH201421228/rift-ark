...
> | 개인정보처리방침 = "아무것도 수집하지 않습니다" | ★ **전면 교체** (`56 §4.5` 가 전문) | §7.2 |
...
> ### 방침 **전문은 여기에 두지 않는다.**
> ★★ **한국어 정본 = [`../legal/privacy-policy-ko.md`](../legal/privacy-policy-ko.md)**
> (2026-08-08 작성 · 그대로 게시할 수 있는 파일이다. `<법적 성명>` 한 곳만 채운다).
> **영문판은 아직 초안 상태**로 [`56 §4.5`](56-admob-rewarded-integration.md) 에 있고,
> 거기 적힌 **세 가지 오류가 정정되지 않았다** — 그대로 게시하지 않는다.
> 초안이 왜 틀렸는지(존재하지 않는 설정 화면 · 동의 거부 시 동작 · iOS ATT)는
> `56 §4.5` 의 대조표에 있다.
...
> 한 줄만 고치면 나머지 문장들이 서로 모순된다. `56 §4.5` 가 **통째로 대체하는
> 전문**을 주는 이유가 그것이다.
...
GitHub 에 `riftark-privacy` 저장소를 만들고 `index.md` 로 두면
`https://<계정>.github.io/riftark-privacy/` 가 공개 URL이 된다. **무료이고 영구적이다.**
영문판은 같은 저장소의 `/en/` 로 둔다 (`53 §5.1`).
