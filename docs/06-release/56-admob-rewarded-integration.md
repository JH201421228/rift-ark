...
| 골드 2배는 경제를 그 자리에서 2배로 민다 | **참 — 실측으로 확인됐고 더 나빴다 (+110%)** | 배수 2.0 은 **넣을 수 없다.** 경제를 지키는 최대치는 총수입 **+19%** 다 (§6.3) |
...
`FE/android/app/src/main/AndroidManifest.xml` 의 `<application>` 안에 넣는다:
...
그리고 `FE/android/app/src/main/res/values/strings.xml` 에:

```xml
<string name="admob_app_id">ca-app-pub-0000000000000000~0000000000</string>
```

| 왜 `strings.xml` 을 경유하는가 | 이유 |
|---|---|
| 매니페스트에 직접 박으면 | 값이 **빌드 타입과 무관하게 하나**다. 디버그/릴리스를 나눌 수 없다 |
| `strings.xml` 경유 | `src/debug/res/values/strings.xml` 로 **디버그 전용 앱 ID**(구글 테스트 앱 ID)를 덮어쓸 수 있다 |
| 구글 테스트 앱 ID | Android `ca-app-pub-3940256099942544~3347511713` · iOS `ca-app-pub-3940256099942544~1458002511` [추정 — 구글 공개 샘플 값. 콘솔 문서에서 확인할 것] |
...
현재 **약 32MB** [출처 `55 §3`]. Google Mobile Ads + UMP 추가분은 **다운로드
기준 +2~5MB** 로 본다 [추정 — 인디 사례 관측 범위].

> ⚠ **이 저장소의 릴리스 빌드는 `minifyEnabled false` 다** [실측
> `FE/android/app/build.gradle`]. R8 이 꺼져 있으면 광고 SDK 의 미사용 클래스가
> 그대로 남아 증가폭이 상단(+5MB 이상)으로 간다. **광고를 붙이는 김에 R8 을
> 켜는 것을 검토한다** — 다만 R8 은 리플렉션을 쓰는 SDK 에서 런타임 크래시를
> 만들 수 있으므로, **켠 빌드로 실기기에서 광고를 한 번 띄워 보기 전까지는
> 켰다고 말하지 않는다.**
...
#### 한국어판 (`https://<계정>.github.io/riftark-privacy/`)
...
| 광고 식별자 | Android 광고 ID, App set ID, iOS 광고 식별자(IDFA) | 광고 게재, 빈도 제한, 부정행위 방지 |
...
- **EEA·영국·스위스**: 앱 최초 실행 시 Google User Messaging Platform 을 통해
  동의를 요청합니다. 동의하지 않으면 개인화되지 않은 광고만 게재됩니다.
  동의 내용은 앱의 **설정 > 개인정보 옵션**에서 언제든지 변경할 수 있습니다.
- **iOS**: 시스템의 앱 추적 투명성(ATT) 프롬프트를 통해 허용 여부를 선택할 수
  있으며, iOS 설정 > 개인정보 보호 및 보안 > 추적에서 언제든 변경할 수 있습니다.
- **미국 일부 주**: 해당 지역 사용자에게는 옵트아웃 선택지가 제공되며, 옵트아웃 시
  제한된 데이터 처리가 적용되어 개인화되지 않은 광고만 게재됩니다.
- **광고 식별자 초기화**: Android 는 설정 > Google > 광고, iOS 는 설정 >
  개인정보 보호 및 보안 > 추적에서 초기화하거나 삭제할 수 있습니다.
- **광고를 보지 않는 선택**: 보상형 광고는 전적으로 선택 사항입니다. 광고를 한 번도
  보지 않아도 게임의 모든 콘텐츠를 끝까지 진행할 수 있습니다.
...
**2026-08-07 실측 — 전부 구현되어 있다.** (`@capacitor-community/admob` 만 아직
미설치다 — 계정과 광고 단위 ID 가 없기 때문이고, 어댑터가 동적 import 로 감싸므로
없어도 앱이 죽지 않는다.)
...
tools/lib/f2p-power.mjs          adGold() — 하네스가 광고를 아는 자리
tools/calibrate-economy.mjs      광고 켠 곡선 + 증가분 게이트
```
...
> ⚠ **`ads.json:_doc` 이 아직 `store/slices/adSlice.js` 를 가리킨다** [실측 2026-08-07].
> 실제 구현은 `metaSlice.js` 다. **같은 사실을 두 곳에 적으면 반드시 갈라진다** —
> 이 저장소의 단일 실패 유형이고, 지금이 그 한 줄이다. 데이터 파일의 주석을 고친다.
...
**해야 하는 것:** `showRewardVideoAd()` 의 프라미스를
`RewardAdPluginEvents.Dismissed` · `FailedToShow` 리스너와 **경주시킨다.**
먼저 도착한 쪽이 답이고, 이긴 뒤에는 양쪽 리스너를 반드시 해제한다.

| 무엇이 먼저 오는가 | 결과 |
|---|---|
| `showRewardVideoAd()` resolve (= `Rewarded`) | **보상 지급** |
| `Dismissed` | 보상 없음 |
| `FailedToShow` | 보상 없음 |
...
| `tools/calibrate-economy.mjs` 의 증가분 게이트 | **있다** | 광고가 총 가용 골드를 **+20% 넘게 밀면 실패** (§6.2) |
...
### 6.0 먼저: 55 의 표를 정확히 읽는다

`55 §2` 는 `npm run economy` 의 출력에서 **파워 비**(0.79~0.87)를 골드 비로 옮겨
적었다. 2026-08-07 재실행 [실측]:

| 스테이지 | 필요골드 | 가용골드 | **골드 비** | (참고) 파워 비 |
|---:|---:|---:|---:|---:|
...
**정정해도 55 의 결론은 그대로이고 오히려 더 강해진다.**
골드 여유는 **1.15~1.30배**뿐이다 — 즉 무과금 플레이어는 목표 성장을 사고 나면
**15~30% 밖에 안 남는다.** 그리고 40–60 구간의 **파워 비 0.79~0.83** 이 이 게임의
벽이다. 그 벽은 **편성 퍼즐이어야 한다** (`CLAUDE.md` 설계 결정 5 · 게이트 B4).

**골드를 더 주면 레벨을 더 사고, 레벨은 파워다. 광고는 정확히 그 벽을 지운다.**

### 6.1 하네스가 광고를 알게 만들었다 — 그래야 게이트가 거짓말을 하지 않는다
...
이것은 이 저장소가 이름을 붙여 놓은 실패 유형이다 — **선언했는데 아무도 읽지 않는 것.**
...
| `tools/lib/f2p-power.mjs:availableGold(s, {ads:true})` | 광고 켠 곡선 |
| `tools/calibrate-economy.mjs` | **광고 켠 곡선을 나란히 출력**하고 증가분 게이트를 건다 |

> ★★ **모델은 최악의 경우로 잡았다.** 플레이어가 광고를 *가장 최근(= 가장 비싼)
> 스테이지*에 몰아 쓴다고 본다. 초반 스테이지에 붙는다고 모델링하면 위험을
> 과소평가하고, **검증은 언제나 나쁜 쪽을 봐야 한다.**

### 6.2 게이트: 광고가 총 가용 골드를 +20% 넘게 밀면 실패

**그 20% 는 고른 값이 아니라 계산된 값이다.**

```
가장 빡빡한 지점은 스테이지 40 — 파워 여유가 0.83 이다.
(1 − 0.83) / 0.83 = +20.5 %
⇒ 총수입이 그 이상 늘면 여유가 1.0 에 닿고, 40–60 구간이 골드로 풀린다.
```

> ★ **절대 여유가 아니라 증가분을 본다.** 처음에는 "광고 켠 여유 ≤ 1.5배"로
> 두었는데, 그러면 **스테이지 100 이 언제나 실패한다** — 거기는 광고 없이도
> 여유가 2.12 다. 광고와 무관한 실패를 광고 탓으로 신고하는 검사기는 아무도
> 믿지 않게 되고, **그 순간 검사기가 죽는다.**

### 6.3 ★★★ 실측 스윕 — "클리어 골드 2배"는 이 게임에서 불가능하다

`rewardMult` × `dailyViews` 를 훑은 결과 [실측 2026-08-07]:
...
**55 §2 의 계산이 실측으로 확인됐고, 실제로는 더 나빴다.** 55 는 "골드 2배 = 총수입
2배"라고 썼는데, 하네스는 **+110%** 를 냈다 — 골드가 스테이지마다 지수로 커지므로
플레이어가 광고를 최신 스테이지에 몰아 쓰면 배수보다 더 밀린다.

> ★★ **`minStage`(6 / 20 / 40)는 결과를 바꾸지 못했다.**
> 플레이어가 광고를 가장 골드가 큰 최신 스테이지에 몰아 쓰기 때문이고, 하네스도
> 그 최악의 경우로 계산한다. **초반 스테이지를 제외하는 것은 UX 이유이지
> 경제 이유가 아니다** — 신규 플레이어가 게임을 배우기도 전에 광고부터 만나지
> 않게 하는 것. `ads.json:_stageRangeDoc` 의 설명을 그 뜻으로 읽는다.

### 6.4 채택된 값과 그 대가

`ads.json` 현재 값 [실측]: `rewardMult` **1.3** · `dailyViews` **1** · `minStage` **6**.

| | |
|---|---|
| 경제 | ✅ 통과 (+19%) |
| 광고 수익 | ★ **하루 노출 1회.** 55 §1.2 의 "설치당 17회 노출" 가정이 **성립하지 않는다** |
| 결과 | 경제는 지켰지만 **광고 수익이 거의 안 나온다** |

**이것이 55 가 말한 출구 없음이다** — *"경제를 지키면 광고가 안 벌고, 광고를 벌게
하면 경제가 깨진다."* 실측은 그 문장을 반박한 것이 아니라 **확인**했다.

### 6.5 ★ 선택지 셋 — 사용자가 고른다

| | 무엇 | 경제 게이트 | 광고 수익 | 대가 |
|---|---|---|---|---|
| **A** | **안전한 골드 보상 (현재 구현)** — ×1.3 · 하루 1회 | ✅ 통과 | ★ **거의 없음** (하루 1회) | 없음. 지금 상태 |
| **B** | **2배를 위해 경제를 재설계** — 100 스테이지 `difficultyMult` · `goldPerStage*` 재보정 | 재작성 | 높음 | `economy` + `balance:check`(300시드 **약 2시간**) 전면 재검증. **사실상 밸런싱 한 사이클** |
| **C** | ★ **골드가 아닌 보상** — 예: 그 판의 **각인 리롤 +1** | **해당 없음** | ★ **가장 높음** | 리롤 축이 기록보관소 시설과 겹친다 |

#### ★ 권장은 C 다

**광고 수익은 노출 수에 비례한다.** A 는 하루 1회로 묶여 있고, C 는 **매 전투마다
제안할 수 있다.** 그리고 C 는 골드 곡선을 **아예 건드리지 않으므로 경제 게이트를
통과할 필요조차 없다** — §6.2 의 +20% 상한이 적용될 대상이 없다.

| C 가 이기는 이유 | |
|---|---|
| 노출 | 하루 1회 → **전투마다.** 55 §1.2 의 17회 가정이 되살아난다 |
| 경제 | `availableGold` 에 아무것도 더하지 않는다. `adGold()` 가 0 이다 |
| 설계 결정 5 | 벽은 여전히 **편성 퍼즐**이다. 리롤은 편성을 바꾸지 않고 그 판의 선택지만 넓힌다 |
| 확률형 | 리롤 자체가 확정 지급이면(3지선다를 한 번 더 뽑는 것) **여전히 확률형이 아니다** — 각인 드래프트는 이미 그 구조다 |
| 되돌리기 | 그 판이 끝나면 사라진다. 세이브에 영구 효과가 남지 않는다 |

> ⚠ **다만 리롤은 별도 판단이 필요하다.** 각인 리롤은 **기록보관소 시설**과 축이
> 겹친다 — 메타 성장이 이미 "각인 선택지 수"를 손잡이로 쓰고 있고
> (`CLAUDE.md` 설계 결정 4), 광고가 같은 축을 밀면 **시설 투자의 의미가 희석된다.**
> "광고를 보면 시설 없이도 같은 것을 얻는다"가 되면, 그것은 골드 벽이 아니라
> **성장 벽을 광고로 넘는 것**이고 이름만 다른 같은 문제다.
> → **그 균형은 이 문서가 결정하지 않는다.** 리롤 외의 후보(그 판 한정 마나 시작치 ·
> 부활 1회 · 결과 화면의 장식)도 같은 검토가 필요하다.

> ★★ **B 를 고른다면 순서가 있다.** 경제 재설계는 `difficultyMult` 100개를 다시
> 잡는 일이고, 그 뒤에 `balance:check` 300시드가 **약 2시간** 돈다. 현재
> **하드 게이트 2항(B3 · BN3)이 이미 사용자 결정 대기 상태**다
> (`CLAUDE.md` · `22-nightmare.md` §0-A.1). **미결 게이트 위에 재설계를 얹지 않는다** —
> 무엇이 원인인지 구분할 수 없게 된다.

### 6.6 통과 기준 (어느 선택지든 공통)

| 게이트 | 조건 |
|---|---|
| `npm run economy` | 광고 켠 총 가용 골드 증가분 **≤ +20%** (전 구간) |
| `npm run economy` | 광고 **끈** 상태에서 여전히 전 구간 통과 — 광고 없이도 도달 가능해야 한다 |
| `npm run balance:check` | **B4**(현재 파워 + 올바른 편성 = 클리어 가능)가 광고 켠 파워에서도 통과 |
...
- [ ] `ads.json:enabled = false` · 플러그인 **미설치**
...
- [ ] `npm i @capacitor-community/admob@7.2.0` (§2.1 — **8.0.0 아님**)
- [ ] `AndroidManifest.xml` 에 `APPLICATION_ID` meta-data (§2.3) ← **없으면 앱이 죽는다**
- [ ] `strings.xml` + `src/debug/res/values/strings.xml`
- [ ] iOS `Info.plist` — `GADApplicationIdentifier` · `SKAdNetworkItems` · `NSUserTrackingUsageDescription` (§2.5)
- [ ] `ads.json:units.android` / `units.ios` 에 **실제 단위 ID** 기입
- [ ] §5.3 의 셋을 전부 처리 (프라미스 경주 · `canRequestAds` · `immersiveMode`)
- [ ] 설정 화면에 **개인정보 옵션** 조건부 항목 (§3.2)

### 3단계 — 경제 게이트 (여기를 통과하지 못하면 켜지 않는다)

- [x] `f2p-power.mjs:adGold()` — 하네스가 광고를 안다 (§6.1) **[완료 2026-08-07]**
- [x] `calibrate-economy.mjs` 증가분 게이트 (+20%) (§6.2) **[완료]**
- [x] 배수 × 일일횟수 스윕 → `rewardMult` 1.3 · `dailyViews` 1 채택 (§6.3) **[완료]**
- [ ] ★ **§6.5 의 선택지 A / B / C 를 사용자가 고른다** ← **여기가 미결이다**
- [ ] `npm run economy` — 광고 켠 증가분 ≤ +20%
- [ ] `npm run economy` — 광고 **끈** 상태도 전 구간 통과
- [ ] `npm run balance:check` — B4 통과
- [ ] `npm run playthrough` — 광고 0회로 100 스테이지 완주
- [ ] `ads.json:_doc` 의 `adSlice.js` → `metaSlice.js` 정정 (§5.1)

> ★ `SAVE_VERSION` 은 **올리지 않았다** — 시청 기록이 `meta.ads` 안에 있어
> 정규화 한 줄이 옛 세이브를 채운다 (§5.1). **C 를 골라 판 한정 보상으로 가면
> 이 상태조차 세이브에 남길 필요가 없어진다.**
...
| 미결 | 왜 |
|---|---|
| ★★★ **§6.5 의 선택지 A / B / C** | **사용자 결정.** A(현재 · 하루 1회 · 수익 거의 없음) · B(경제 재설계 · 밸런싱 한 사이클) · C(골드가 아닌 보상 · 권장). **이 결정 전에는 `ads.json:enabled` 를 켜지 않는다** |
| C 를 고를 경우 무엇을 줄 것인가 | 각인 리롤은 **기록보관소 시설과 축이 겹친다** (§6.5). 리롤 · 마나 시작치 · 부활 1회 · 장식 중 어느 것이 성장 벽을 광고로 넘게 만들지 않는가 |
